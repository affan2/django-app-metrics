...
class MetricCreationTests(TestCase):
    ...
    # These need celery to run correctly.
    ...
    #     # This is not used in this function so it seems pointless.
...
# Has a  dependency on settings and it was not properly configured as user was not created and
# passed to metric item constructor.
# class MixpanelCommandTest1(TestCase):
#     """ Test out our management command noops """
#
#     def setUp(self):
#         self.user1 = User.objects.create(username='Bob Jones')
#         new_metric = Metric.objects.create(name='foo bar')
#         i = MetricItem.objects.create(metric=new_metric, user=self.user1)
